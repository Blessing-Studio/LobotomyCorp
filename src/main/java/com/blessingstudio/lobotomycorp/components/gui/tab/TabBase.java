...
public abstract class TabBase extends GuiButton {
    ...
    public TabBase(int id, int posX, int posY, ItemStack renderStack) {
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
        int newPotionOffset = 0;//TabService.getPotionOffsetNEI();
        GuiScreen screen = FMLClientHandler.instance().getClient().currentScreen;
        if (screen instanceof GuiInventory) {
            newPotionOffset += TabService.getRecipeBookOffset((GuiInventory) screen) - TabService.recipeBookOffset;
        }
