...
import static com.blessingstudio.lobotomycorp.components.data.NbtData.LEARNING_DONE;

public class CustomNbtDataService {
    public static NBTTagCompound getNBT(ItemStack stack) {
        AtomicReference<NBTTagCompound> nbt = new AtomicReference<>(stack.getTagCompound());
        if (nbt.get() == null) {
            nbt.set(new NBTTagCompound());
            stack.setTagCompound(nbt.get());
        }
        return nbt.get();
    }

    public static NBTTagCompound getNBT(Entity entity) {
        AtomicReference<NBTTagCompound> nbt = new AtomicReference<>(entity.getEntityData());
        return nbt.get();
    }
    ...
    public static boolean GetIsLearned(EntityPlayer player, int skillID) {
        ...
        return Arrays.binarySearch(learnt, skillID) >= 0;
    }

    public static void SetIsLearned(EntityPlayer player, int skillID, boolean val) {
        int[] learnt = CustomNbtData.getPlayerIdeallandIntArraySafe(player, LEARNING_DONE);
        ...
        if (oldIndex >= 0) {
            ...
            if (val) {
                ...
                CustomNbtData.setPlayerIdeallandTagSafe(player, LEARNING_DONE, newLearnt);
            }else {
                return;
            }
        }
